...
namespace cvx
{
    ...
    inline void cross(cv::Mat& img, cv::Point2f centre, float radius, const cv::Scalar& colour, int thickness = 1, int lineType = cv::LINE_AA, int shift = 8)
    {
        cvx::line(img, centre + cv::Point2f(-radius, -radius), centre + cv::Point2f(radius, radius), colour, thickness, lineType, shift);
        cvx::line(img, centre + cv::Point2f(-radius, radius), centre + cv::Point2f(radius, -radius), colour, thickness, lineType, shift);
    }
    inline void plus(cv::Mat& img, cv::Point2f centre, float radius, const cv::Scalar& colour, int thickness = 1, int lineType = cv::LINE_AA, int shift = 8)
    {
